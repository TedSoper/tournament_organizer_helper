...
$("document").ready(function(){
	...
	var playerData={"playerId":playerId,"firstName":firstName,
	"lastName":lastName,"army":army,"email":email,"time":time,
	"battlePoints":"", "sportsmanship":"", "painting":""};
...
function buildRoundTable(tableNumber, roundPairings){
	...
	$("#roundResults").removeClass("hide").find("h4").text("Round " + round);
	$("#roundResults h4").val("Round " + round);
	$("#roundResults").append($resultTemplate.attr('id', "round-" + round + "-" + adjTableNumber));
	

	$resultTemplate.find(".panel-title").text("Round " + round + " - Table " + adjTableNumber);
...
function  updateStandings(roundEntryData){
	...
	if($.isNumeric(playerOneData.battlePoints)){
		playerOneData.battlePoints 	= parseInt(playerOneData.battlePoints) 
		+ parseInt(roundEntryData[0].battlePoints);
	} else{
		playerOneData.battlePoints 	= parseInt(roundEntryData[0].battlePoints);
	}

	if($.isNumeric(playerOneData.sportsmanship)){
		playerOneData.sportsmanship 	= parseInt(playerOneData.sportsmanship) 
		+ parseInt(roundEntryData[0].sportsmanship);
	} else{
		playerOneData.sportsmanship 	= parseInt(roundEntryData[0].battlePoints);
	}
	
	if($.isNumeric(playerOneData.painting)){
		playerOneData.painting 	= parseInt(playerOneData.painting) 
		+ parseInt(roundEntryData[0].painting);
	} else{
	...
	if($.isNumeric(playerTwoData.battlePoints)){
		playerTwoData.battlePoints 	= parseInt(playerTwoData.battlePoints) 
		+ parseInt(roundEntryData[1].battlePoints);
	} else{
		playerTwoData.battlePoints 	= parseInt(roundEntryData[1].battlePoints);
	}

	if($.isNumeric(playerTwoData.sportsmanship)){
		playerTwoData.sportsmanship 	= parseInt(playerTwoData.sportsmanship) 
		+ parseInt(roundEntryData[1].sportsmanship);
	} else{
		playerTwoData.sportsmanship 	= parseInt(roundEntryData[1].battlePoints);
	}
	
	if($.isNumeric(playerTwoData.painting)){
		playerTwoData.painting 	= parseInt(playerTwoData.painting) 
		+ parseInt(roundEntryData[1].painting);
	} else{
	...
	if(roundStandings === null){
		roundStandings=[{"playerId":playerOneData.playerId,"battlePoints":playerOneData.battlePoints},
		{"playerId":playerTwoData.playerId,"battlePoints":playerTwoData.battlePoints}];
	} else{
		roundStandings.push({"playerId":playerOneData.playerId,"battlePoints":playerOneData.battlePoints});
		roundStandings.push({"playerId":playerTwoData.playerId,"battlePoints":playerTwoData.battlePoints});
	}

	roundStandings = roundStandings.sort(function(a,b){return a.battlePoints < b.battlePoints;});
